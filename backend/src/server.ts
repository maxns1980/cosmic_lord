...
const initializeWorld = async () => {
    ...
    if (!data) {
        ...
        if (!worldState.npcStates || Object.keys(worldState.npcStates).length < TOTAL_NPC_COUNT) {
             ...
             const { error: updateError } = await (supabase.from('world_state') as any).update({ state: worldState as unknown as Json }).eq('id', 1);
            if (updateError) {
...
app.post('/api/signup', async (req, res) => {
    ...
    try {
        ...
        const { data: worldData, error: worldError } = await (supabase.from('world_state') as any).select('state').eq('id', 1).single();
        ...
        const { error: worldSaveError } = await (supabase.from('world_state') as any).update({ state: worldState as unknown as Json }).eq('id', 1);
        if (worldSaveError) {
...
const loadCombinedGameState = async (userId: string): Promise<GameState | null> => {
    const { data: playerData, error: playerError } = await (supabase.from('player_states') as any).select('*').eq('user_id', userId).single();
    const { data: worldData, error: worldError } = await (supabase.from('world_state') as any).select('*').eq('id', 1).single();
    ...
    if (updatedWorldState.lastGlobalNpcCheck > lastNpcCheckBefore) {
        const { error } = await (supabase.from('world_state') as any).update({ state: updatedWorldState as unknown as Json }).eq('id', 1);
        if (error) {
...
const saveStates = async (userId: string, gameState: GameState) => {
    ...
    const playerSavePromise = (supabase.from('player_states') as any).update({ state: playerState as unknown as Json }).eq('user_id', userId);
    const worldSavePromise = (supabase.from('world_state') as any).update({ state: worldState as unknown as Json }).eq('id', 1);
