...
export {}; // Make this file a module to scope global declarations
...
app.post('/api/signup', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const newUser: Database['public']['Tables']['users']['Insert'] = { username, password };
        ...
        const newPlayerStateRecord: Database['public']['Tables']['player_states']['Insert'] = { user_id: username, state: newPlayerState as Json };
        const { error: insertStateError } = await supabase
            .from('player_states')
            .insert(newPlayerStateRecord);
...
const loadCombinedGameState = async (userId: string): Promise<GameState | null> => {
    ...
    if (updatedWorldState.lastGlobalNpcCheck > lastNpcCheckBefore) {
        const { error } = await supabase.from('world_state').update({ state: updatedWorldState as Json }).eq('id', 1);
        if (error) {
