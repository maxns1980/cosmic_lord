...
export const calculateProductions = (gameState: GameState) => {
    ...
    const homeworld = allColonies.reduce((oldest: Colony, current: Colony) => 
    ...
    for (const planet of allColonies) {
        ...
        let solarPlantProduction = (BUILDING_DATA[BuildingType.SOLAR_PLANT].production?.((planet as Colony).buildings[BuildingType.SOLAR_PLANT]) ?? 0) * energyTechBonus;
        ...
        if ((planet as Colony).buildings[BuildingType.FUSION_REACTOR] > 0) {
            planetEnergyProduction += (BUILDING_DATA[BuildingType.FUSION_REACTOR].production?.((planet as Colony).buildings[BuildingType.FUSION_REACTOR]) ?? 0) * energyTechBonus;
        }

        const satelliteData = ALL_SHIP_DATA[ShipType.SOLAR_SATELLITE];
        planetEnergyProduction += ((planet as Colony).fleet[ShipType.SOLAR_SATELLITE] || 0) * (satelliteData.energyProduction || 0);
       
        if (solarFlare.status === SolarFlareStatus.POWER_BOOST) {
            planetEnergyProduction *= 1.5;
        }

        const planetEnergyConsumption = (Object.keys((planet as Colony).buildings) as BuildingType[]).reduce((total, type) => {
            const buildingInfo = BUILDING_DATA[type as BuildingType];
            if (type !== BuildingType.FUSION_REACTOR && (planet as Colony).buildings[type as BuildingType] > 0) {
               return total + (buildingInfo.energyConsumption?.((planet as Colony).buildings[type as BuildingType]) ?? 0);
            }
            return total;
        }, 0);
        ...
        let metalProd = (BUILDING_DATA[BuildingType.METAL_MINE].production?.((planet as Colony).buildings[BuildingType.METAL_MINE]) ?? 0) * efficiency;
        let crystalProd = (BUILDING_DATA[BuildingType.CRYSTAL_MINE].production?.((planet as Colony).buildings[BuildingType.CRYSTAL_MINE]) ?? 0) * efficiency;
        let deuteriumProd = (BUILDING_DATA[BuildingType.DEUTERIUM_SYNTHESIZER].production?.((planet as Colony).buildings[BuildingType.DEUTERIUM_SYNTHESIZER]) ?? 0) * efficiency;
        
        if ((planet as Colony).buildings[BuildingType.FUSION_REACTOR] > 0) {
            deuteriumProd -= BUILDING_DATA[BuildingType.FUSION_REACTOR].deuteriumConsumption?.((planet as Colony).buildings[BuildingType.FUSION_REACTOR]) ?? 0;
        }

        if ((planet as Colony).id !== homeworld.id) {
            ...
            if ((planet as Colony).specialization === PlanetSpecialization.DEUTERIUM_BOOST) {
