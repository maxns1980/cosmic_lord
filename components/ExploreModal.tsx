...
interface ExploreModalProps {
    ...
    onSend: (missionFleet: Fleet, targetCoords: string, durationSeconds: number, fuelCost: number) => void;
    onClose: () => void;
}
...
const ExploreModal: React.FC<ExploreModalProps> = ({ targetCoords, fleet, resources, research, shipLevels, activeLocationId, activeBoosts, onSend, onClose }) => {
    ...
    useEffect(() => {
        ...
        const researchVessels = missionFleet[ShipType.RESEARCH_VESSEL] || 0;
        const explorationDurationHours = researchVessels > 0 ? 2 + (researchVessels - 1) * 0.5 : 0;
        setExplorationTime(explorationDurationHours * 3600);
    ...
    const handleSendClick = () => {
        onSend(missionFleet, targetCoords, travelTime, fuelCost);
    };
    ...
    return (
        ...
        >
            <div 
                className="bg-gray-800 border-2 border-teal-500 rounded-2xl shadow-2xl max-w-2xl w-full text-left transform transition-all relative"
            ...
            >
                <div className="p-8">
                    ...
                    <h2 className="text-3xl font-bold text-teal-300 mb-6 border-b border-teal-700 pb-3 flex items-center gap-3">
                    ...
                    <div className="space-y-4">
                        ...
                        {!hasResearchVessel && hasShipsSelected && (
                            <p className="text-center text-amber-400 text-sm p-2 bg-amber-900 bg-opacity-50 rounded-md">
                                Do misji "Eksploruj" wymagany jest co najmniej jeden Okręt Badawczy.
                            </p>
                        )}
                        ...
                        <div className="grid grid-cols-3 gap-4 text-center bg-gray-900 p-3 rounded-lg">
                            ...
                            <div>
                                <p className="text-sm text-gray-400">Czas eksploracji</p>
                    ...
                    <div className="mt-8 flex gap-4">
                        ...
                        <button
                            ...
                            className="flex-1 px-6 py-2 text-base font-bold text-white rounded-md shadow-md transition-colors duration-300 bg-teal-600 hover:bg-teal-500 disabled:bg-gray-500 disabled:cursor-not-allowed"
                        >
                            Wyślij na Eksplorację
                        </button>
                    </div>
                </div>
            </div>
